...
func max*(clr1, clr2: ColorObj): ColorObj =
  ...
  clr(max(clr1.r, clr2.r), max(clr1.g, clr2.g), max(clr1.g, clr2.g), max(clr1.a, clr2.a))

func min*(clr1, clr2: ColorObj): ColorObj =
  ## Returns minimal color.
  clr(min(clr1.r, clr2.r), min(clr1.g, clr2.g), min(clr1.g, clr2.g), min(clr1.a, clr2.a))
...
func pow*(clr: ColorObj, power: float): ColorObj =
  ## Returns positivie color.
  clr(pow(clr.r, power), pow(clr.g, power), pow(clr.b, power), pow(clr.a, power))

func bright*(clr: ColorObj): float =
...
func blend*(clr1, clr2: ColorObj, mode: BlendMode = bmNormal): ColorObj =
  ## Blends two colors.
  case mode
  of bmNormal:
    mix(clr1, clr2)
...
iterator walk(clr1, clr2: ColorObj, step_count: int = 100): ColorObj {.used.} =
